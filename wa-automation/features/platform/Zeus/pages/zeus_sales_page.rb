...
class ZeusSalesPage
    ...
    @BROWSER = browser
    @BROWSER.goto $ZEUS[0..-7]
		@BTN_FEATURES = @BROWSER.span(:text, 'Features')
		@BTN_PRICING = @BROWSER.span(:text, 'Pricing')
		@BTN_CONTACT = @BROWSER.span(:text, 'Contact')
		@BTN_LOGIN = @BROWSER.span(:text, 'Login')
	...
	def is_visible(page)
		case page
		when 'main'
			@BROWSER.span(:class, %w(Employee Engagement.)).wait_until_present
			@BROWSER.span(:class, %w(Every Single Day..)).wait_until_present
			@BTN_FEATURES.wait_until_present
			@BTN_PRICING.wait_until_present
			@BTN_CONTACT.wait_until_present
  ...
  # First screen of Zeus self-signup process
  def signup_step_1
    @BROWSER.span(:text, /Register your/).wait_until_present
    @BROWSER.a(:text, '1 month Premium free trial').wait_until_present
    @BROWSER.span(:text, "Tell us about your company and we’ll get you set up in a flash!").wait_until_present
  ...
  # Second screen of Zeus self-signup process
  def signup_step_2
    ...
    @BROWSER.input(:placeholder, 'e.g 0203 567 5900').send_keys USER_PROFILE[:new_admin_user][:phone]
